...
{
   gfloat score;
   int level;			/* length of level arg to mate_score_log
				 * including null term */
   gboolean ordering;
};
...
static gchar *
mate_get_score_file_name (const gchar * progname, const gchar * level)
{
   if (level)
     return g_strconcat (LIBMATE_LOCALSTATEDIR,
...
static gpointer
mate_score_child_thread (gpointer data)
{
  int *a = data;
 ...
 */
gint
mate_score_get_notable (const gchar * gamename,
			 const gchar * level,
			 gchar *** names,
			 gfloat ** scores,
			 time_t ** scoretimes)
