...
na_fixed_tip_position (NaFixedTip *fixedtip)
{
  ...
  gdk_window_get_origin (gtk_widget_get_window (fixedtip->priv->parent), &root_x, &root_y);
		...
		parent_width = gdk_window_get_width(GDK_WINDOW(gtk_widget_get_window(fixedtip->priv->parent)));
		parent_height = gdk_window_get_height(GDK_WINDOW(gtk_widget_get_window(fixedtip->priv->parent)));
	#else
		gdk_drawable_get_size(GDK_DRAWABLE(gtk_widget_get_window(fixedtip->priv->parent)), &parent_width, &parent_height);
	#endif
