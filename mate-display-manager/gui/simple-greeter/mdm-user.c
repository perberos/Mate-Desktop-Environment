 ...
 **/
void
_mdm_user_update_from_pwent (MdmUser             *user,
                             const struct passwd *pwent)
{
        ...
        if (g_strcmp0 (pwent->pw_name, user->user_name) != 0) {
                ...
                if (pwent->pw_name != NULL) {
                        ...
                        user->icon_file = g_build_filename (MDM_CACHE_DIR, pwent->pw_name, "face", NULL);
...
/**
 * _mdm_user_update_login_frequency:
 * @user: the user object to update
 ...
 **/
void
_mdm_user_update_login_frequency (MdmUser *user,
...
/**
 * mdm_user_is_logged_in:
 * @user: a #MdmUser
 *
 * Returns whether or not #MdmUser is currently logged in.
 *
 * Returns: %TRUE or %FALSE
 */
gboolean
mdm_user_is_logged_in (MdmUser *user)
{
        return user->sessions != NULL;
}

/**
 * mdm_user_render_icon:
 * @user: a #MdmUser
 ...
 */
const char *
mdm_user_get_icon_file (MdmUser *user)
{
        g_return_val_if_fail (MDM_IS_USER (user), NULL);
 ...
 */
const char *
mdm_user_get_object_path (MdmUser *user)
{
        g_return_val_if_fail (MDM_IS_USER (user), NULL);
...
/**
 * mdm_user_get_primary_session_id:
 * @user: a #MdmUser
 *
 ...
 */
const char *
mdm_user_get_primary_session_id (MdmUser *user)
{
        if (!mdm_user_is_logged_in (user)) {
...
update_info (MdmUser *user)
{
        ...
        if (call == NULL) {
                g_warning ("MdmUser: failed to make GetAll call");
                goto failed;
        }
...
changed_handler (DBusGProxy *proxy,
                 gpointer   *data)
{
        MdmUser *user = MDM_USER (data);

        update_info (user);
}

/**
 * _mdm_user_update_from_object_path:
 * @user: the user object to update.
 ...
 **/
void
_mdm_user_update_from_object_path (MdmUser    *user,
 ...
 */
gboolean
mdm_user_is_loaded (MdmUser *user)
{
