...
static gboolean
launch_marco (GdmChooserSession *session)
{
        GError  *error;
        gboolean ret;

        g_debug ("GdmChooserSession: Launching marco");
...
start_settings_daemon (GdmChooserSession *session)
{
        ...
        error = NULL;
        g_spawn_command_line_async (LIBEXECDIR "/mate-settings-daemon --mateconf-prefix=/apps/gdm/simple-chooser/settings-manager-plugins", &error);
        if (error != NULL) {
