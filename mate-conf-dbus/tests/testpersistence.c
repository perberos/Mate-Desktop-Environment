...
static void
sync_and_clear(MateConfEngine* conf)
{
  mateconf_synchronous_sync(conf, NULL);
  mateconf_clear_cache(conf, NULL);
}
...
void
check_bool_storage(MateConfEngine* conf)
{
  GError* err = NULL;
  const gchar** keyp = NULL;
  guint i; 
...
void
check_float_storage(MateConfEngine* conf)
{
  GError* err = NULL;
  const gchar** keyp = NULL;
  guint i; 
      ...
      while (i < n_floats)
        {
          ...
          if (!mateconf_engine_set_float(conf, *keyp, floats[i], &err))
          ...
          else
            {
              ...
              gotten = mateconf_engine_get_float(conf, *keyp, &err);
...
static void
check_list_storage(MateConfEngine* conf)
{
  GError* err = NULL;
  const gchar** keyp = NULL;
  guint i;
      ...
      while (i < n_lists)
        {
          ...
          if (!mateconf_engine_set_list(conf, *keyp, list_types[i], lists[i], &err))
          ...
          else
            {
              ...
              gotten = mateconf_engine_get_list(conf, *keyp, list_types[i], &err);
