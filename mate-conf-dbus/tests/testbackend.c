...
#include <mateconf/mateconf-locale.h>
...
typedef void (* ForeachEntryFunc) (MateConfEntry *entry,
                                   int         depth,
                                   void       *data);
...
static void
foreach_recursive (MateConfSource     *source,
                   const char      *dir_key,
                   int              depth,
...
static void
sync_and_clear (MateConfSource *source)
{
  GError *err;
...
static void
check_unset (MateConfSource *source)
{
  GError* err = NULL;
...
static gboolean
set_string (MateConfSource *source,
            const char  *key,
            const char  *str,
            GError     **err)
{
...
static gboolean
set_bool (MateConfSource *source,
          const char  *key,
          gboolean     v,
          GError     **err)
{
...
static gboolean
set_float (MateConfSource *source,
           const char  *key,
           double       v,
           GError     **err)
{
...
static gboolean
set_list (MateConfSource   *source,
          const char    *key,
          ...
          GError       **err)
{
  ...
  mateconf_value_free (value_list);
  return ret;
}

static MateConfValue*
get_value (MateConfSource *source,
           const char  *key,
           GError     **err)
{
...
static char*
get_string (MateConfSource *source,
            const char  *key,
            GError     **err)
{
  MateConfValue *val;
  ...
  if (val)
    {
      g_assert (val->type == MATECONF_VALUE_STRING);
      s = mateconf_value_steal_string (val);
...
static gboolean
get_bool (MateConfSource *source,
          const char  *key,
          GError     **err)
{
  MateConfValue *val;
  ...
  if (val)
    {
      g_assert (val->type == MATECONF_VALUE_BOOL);
      b = mateconf_value_get_bool (val);
      mateconf_value_free (val);
    }
  else
    {
          ...
          GError     **err)
{
  ...
  if (val)
    {
      g_assert (val->type == MATECONF_VALUE_INT);
      i = mateconf_value_get_int (val);
      mateconf_value_free (val);
    }
  else
    {
...
static double
get_float (MateConfSource *source,
           const char  *key,
           GError     **err)
{
  MateConfValue *val;
  ...
  if (val)
    {
      g_assert (val->type == MATECONF_VALUE_FLOAT);
      d = mateconf_value_get_float (val);
      mateconf_value_free (val);
    }
  else
    {
...
static GSList*
get_list (MateConfSource     *source,
          const char      *key,
          MateConfValueType   list_type,
          GError         **err)
{
...
static void
check_string_storage (MateConfSource *source)
{
  GError* err = NULL;
  const char** keyp = NULL;
...
void
check_bool_storage (MateConfSource *source)
{
  GError* err = NULL;
  const char** keyp = NULL;
  guint i; 
...
void
check_float_storage (MateConfSource *source)
{
  GError* err = NULL;
  const char** keyp = NULL;
  guint i; 
...
void
check_int_storage (MateConfSource *source)
{
  GError* err = NULL;
  const char** keyp = NULL;
  guint i; 
...
static void
compare_lists (MateConfValueType type, GSList* first, GSList* second)
{
  GSList* l1;
  GSList* l2;
  ...
  check (l2 == NULL, "second list too long with list type %s",
         mateconf_value_type_to_string (type));
}

static void
free_list (MateConfValueType type, GSList* list)
{
  GSList* tmp = list;
...
static void
check_list_storage (MateConfSource *source)
{
  GError* err = NULL;
  const char** keyp = NULL;
  guint i;
  MateConfValueType list_types[] = { MATECONF_VALUE_INT, MATECONF_VALUE_INT,
                                  MATECONF_VALUE_STRING, MATECONF_VALUE_STRING,
                                  MATECONF_VALUE_FLOAT, MATECONF_VALUE_FLOAT,
                                  MATECONF_VALUE_BOOL, MATECONF_VALUE_BOOL };
...
static void
print_entry (MateConfEntry *entry,
             int         depth,
             void       *data)
{
  MateConfValue *val;
...
{ 
  ...
  locales = (const char**) mateconf_split_locale (mateconf_current_locale ());
