...
static void
check_add_remove(MateConfListeners* listeners)
{
  guint ids[128];
  guint i;
  const gchar** keyp;
...
static void
check_immediate_remove_after_add(MateConfListeners* listeners)
{
  guint ids[128];
  guint i;
  const gchar** keyp;
  ...
  while (i < 128 && *keyp)
    {
      ...
      check(mateconf_listeners_count(listeners) == 1,
...
static void
check_double_add_remove(MateConfListeners* listeners)
{
  guint ids[128];
  guint i;
  const gchar** keyp;
...
void
notify_callback(MateConfListeners* listeners,
...
check_destroy(void)
{
  ...
  listeners = mateconf_listeners_new();
  ...
  while (i < 128 && *keyp)
    {
      ...
      ids[i] = mateconf_listeners_add(listeners,
                                   *keyp,
                                   d,
  ...
  mateconf_listeners_free(listeners);

  check(destroy_count == i,
        "number of listeners added (%u) doesn't match number destroyed (%u) on MateConfListeners destruction", i, destroy_count);
}
...
main (int argc, char** argv)
{
  ...
  g_assert(mateconf_listeners_count(listeners) == 0);
