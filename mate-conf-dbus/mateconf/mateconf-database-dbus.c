...
#define DATABASE_OBJECT_PATH "/org/mate/MateConf/Database"
...
static void     database_handle_add_notify        (DBusConnection   *conn,
						   DBusMessage      *message,
						   MateConfDatabase    *db);
static gboolean database_remove_notification_data (MateConfDatabase    *db,
...
static ListeningClientData *database_add_listening_client      (MateConfDatabase       *db,
								const gchar         *service);
static void                 database_remove_listening_client   (MateConfDatabase       *db,
...
static void
database_unregistered_func (DBusConnection *connection, MateConfDatabase *db)
{
}
                        ...
                        MateConfDatabase  *db)
{
  ...
  gchar *key;
  gchar *locale;
  MateConfLocaleList *locales;
  gboolean use_schema_default;
  GError *gerror = NULL;
  DBusMessageIter iter;
  
  if (!mateconfd_dbus_get_message_args (conn, message,
  ...
  locales = mateconfd_locale_cache_lookup (locale);
  
  value = mateconf_database_query_value (db, key, locales->list, 
				      use_schema_default,
				      NULL, NULL, NULL, &gerror);

  if (mateconfd_dbus_set_exception (conn, message, &gerror))
    goto fail;
			    ...
			    MateConfDatabase  *db)
{
  ...
  if (value)
    mateconf_dbus_utils_append_entry_values (&iter,
					  key,
					  value,
					  value_is_default,
					  value_is_writable,
				...
				MateConfDatabase  *db)
{
  ...
  value = mateconf_database_query_default_value (db, key, locales->list,
					      NULL,
					      &gerror);
                     ...
                     MateConfDatabase  *db)
{
  ...
  value = mateconf_dbus_utils_get_value (&iter);

  mateconf_database_set (db, key, value, &gerror);
  mateconf_value_free (value);
		       ...
		       MateConfDatabase  *db)
{
  ...
  mateconf_database_unset (db, key, locale, &gerror);
  
  mateconf_database_sync (db, NULL);
                                  ...
                                  MateConfDatabase  *db)
{
  ...
  mateconf_database_recursive_unset (db, key, locale, unset_flags, &gerror);
                            ...
                            MateConfDatabase  *db)
{
  ...
  exists = mateconf_database_dir_exists (db, dir, &gerror);
                                 ...
                                 MateConfDatabase  *db)
{
  ...
  entries = mateconf_database_all_entries (db, dir, 
					locales->list, &gerror);
  ...
  mateconf_dbus_utils_append_entries (&iter, entries);
                              ...
                              MateConfDatabase  *db)
{
  ...
  dirs = mateconf_database_all_dirs (db, dir, &gerror);
                            ...
                            MateConfDatabase  *db)
{
  ...
  mateconf_database_set_schema (db, key, schema_key, &gerror);
		              ...
		              MateConfDatabase  *db)
{
  ...
  mateconf_database_sync (db, &gerror);
...
static gboolean
database_remove_notification_data (MateConfDatabase *db,
				   NotificationData *notification,
				   const char *client)
{
  GList *element;
			       ...
			       MateConfDatabase *db)
{
  ...
  if (notification == NULL || !database_remove_notification_data (db, notification, sender))
    {
      mateconf_log (GCL_DEBUG, _("Notification on %s doesn't exist"),
                 namespace_section);
    }
...
database_add_listening_client (MateConfDatabase *db, 
			       const gchar   *service)
{
  ListeningClientData *client;
  gchar               *rule;
  ...
  dbus_bus_add_match (mateconfd_dbus_get_connection (), rule, NULL);
  g_free (rule);
...
static void
database_remove_listening_client (MateConfDatabase       *db,
				  ListeningClientData *client)
{
  gchar *rule;

  rule = get_rule_for_service (client->service);
  dbus_bus_remove_match (mateconfd_dbus_get_connection (), rule, NULL);
  g_free (rule);
...
void
mateconf_database_dbus_setup (MateConfDatabase *db)
{
  DBusConnection *conn;
  ...
  conn = mateconfd_dbus_get_connection ();
...
void
mateconf_database_dbus_teardown (MateConfDatabase *db)
{
  DBusConnection *conn;
...
const char *
mateconf_database_dbus_get_path (MateConfDatabase *db)
{
  return db->object_path;
}

void
mateconf_database_dbus_notify_listeners (MateConfDatabase    *db,
	  ...
	  for (l = notification->clients; l; l = l->next)
	    {
	      ...
	      dbus_connection_send (mateconfd_dbus_get_connection (), message, NULL);
