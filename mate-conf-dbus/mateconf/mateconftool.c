...
static const GOptionEntry server_entries[] = {
  ...
  { 
    ...
    G_OPTION_ARG_NONE,
    &spawn_mateconfd,
    N_("Launch the config server (mateconfd). (Normally happens automatically when needed.)"),
    NULL
  },
  {
    NULL
...
static const GOptionEntry install_entries[] = {
  ...
  },
  {
    ...
    G_OPTION_ARG_NONE,
    &use_local_source,
    N_("Access the config database directly, bypassing server. Requires that mateconfd is not running."),
    NULL
  },
  {
    ...
    N_("Properly installs schema files on the command line into the database. MATECONF_CONFIG_SOURCE environment variable should be set to a non-default config source or set to the empty string to use the default."),
    NULL
  },
  {
    ...
    N_("Properly uninstalls schema files on the command line from the database. MATECONF_CONFIG_SOURCE environment variable should be set to a non-default config source or set to the empty string to use the default."),
    NULL
  },
  {
    NULL
...
main (int argc, char** argv)
{
  ...
  context = g_option_context_new (_("- Tool to manipulate a MateConf configuration"));
...
do_spawn_daemon(MateConfEngine* conf)
{
  ...
  if (!mateconf_spawn_daemon(&err))
    {
      g_printerr (_("Failed to spawn the config server (mateconfd): %s\n"), 
		  err->message);
      g_error_free(err);
      err = NULL;
    }
