 ...
 * directory.  The magic file in each directory is called %mateconf.xml,
...
typedef struct
{
  MateConfSource source; /* inherit from MateConfSource */
  char *root_dir;
  guint timeout_id;
  MateConfLock* lock;
  MarkupTree *tree;
...
static MateConfSource*   resolve_address (const char        *address,
                                       GError           **err);
static void           lock            (MateConfSource       *source,
                                       GError           **err);
static void           unlock          (MateConfSource       *source,
                                       GError           **err);
static gboolean       readable        (MateConfSource       *source,
                                       const char        *key,
                                       GError           **err);
static gboolean       writable        (MateConfSource       *source,
                                       const char        *key,
                                       GError           **err);
static MateConfValue*    query_value     (MateConfSource       *source,
                                       const char        *key,
                                       const char       **locales,
                                       char             **schema_name,
                                       GError           **err);
static MateConfMetaInfo* query_metainfo  (MateConfSource       *source,
                                       const char        *key,
                                       GError           **err);
static void           set_value       (MateConfSource       *source,
                                       const char        *key,
                                       const MateConfValue  *value,
                                       GError           **err);
static GSList*        all_entries     (MateConfSource       *source,
                                       const char        *dir,
                                       const char       **locales,
                                       GError           **err);
static GSList*        all_subdirs     (MateConfSource       *source,
                                       const char        *dir,
                                       GError           **err);
static void           unset_value     (MateConfSource       *source,
                                       const char        *key,
                                       const char        *locale,
                                       GError           **err);
static gboolean       dir_exists      (MateConfSource       *source,
                                       const char        *dir,
                                       GError           **err);
static void           remove_dir      (MateConfSource       *source,
                                       const char        *dir,
                                       GError           **err);
static void           set_schema      (MateConfSource       *source,
                                       const char        *key,
                                       const char        *schema_key,
                                       GError           **err);
static gboolean       sync_all        (MateConfSource       *source,
                                       GError           **err);
static void           destroy_source  (MateConfSource       *source);
static void           clear_cache     (MateConfSource       *source);
static void           blow_away_locks (const char        *address);


static MateConfBackendVTable markup_vtable = {
  sizeof (MateConfBackendVTable),
  x_shutdown,
  resolve_address,
  lock,
  unlock,
...
{
  mateconf_log (GCL_DEBUG, _("Unloading text markup backend module."));
}

static void
lock (MateConfSource *source,
      GError **err)
{
...
static gboolean
readable (MateConfSource *source,
          const char  *key,
          GError     **err)
{
...
static gboolean
writable (MateConfSource  *source,
          const char   *key,
          GError      **err)
{
...
guint
_mateconf_mode_t_to_mode(mode_t orig)
{
                      ...
                      GError    **err)
{
  ...
  root_dir = mateconf_address_resource (address);

  if (root_dir == NULL)
    {
      mateconf_set_error (err, MATECONF_ERROR_BAD_ADDRESS,
...
get_lock_dir_from_root_dir (const char *root_dir)
{
  ...
  lockdir = mateconf_concat_dir_and_key (root_dir, "%mateconf-xml-backend.lock");
                 ...
                 GError    **err)
{
  ...
  if (g_stat (root_dir, &statbuf) == 0)
    {
      ...
      dir_mode = _mateconf_mode_t_to_mode (statbuf.st_mode);
  ...
  address_flags = mateconf_address_flags (address);  
  if (address_flags)
    {
      iter = address_flags;
      while (*iter)
...

  {
    ...
    if (writable)
      flags |= MATECONF_SOURCE_ALL_WRITEABLE;
    else
      flags |= MATECONF_SOURCE_NEVER_WRITEABLE;

    /* We only do locking if it's writable,
     * and if not using local locks,
     * which is sort of broken but close enough
     */
    if (writable && !mateconf_use_local_locks ())
      {
        gchar* lockdir;
        ...
        lock = mateconf_get_lock (lockdir, err);

        if (lock != NULL)
          mateconf_log (GCL_DEBUG, "Acquired lock directory `%s'", lockdir);
...

  {
    ...
    if (readable)
      flags |= MATECONF_SOURCE_ALL_READABLE;
  }

  if (!(flags & MATECONF_SOURCE_ALL_READABLE) &&
      !(flags & MATECONF_SOURCE_ALL_WRITEABLE))
    {
      mateconf_set_error (err, MATECONF_ERROR_BAD_ADDRESS,
                   ...
                   GError    **err)
{
  ...
  parent = mateconf_key_directory (key);
  ...
  if (dir != NULL)
    {
      ...
      relative_key = mateconf_key_key (key);
...
query_value (MateConfSource *source,
             const char  *key,
             const char **locales,
             char       **schema_name,
             GError     **err)
...
query_metainfo (MateConfSource *source,
                const char  *key,
                GError     **err)
{
  MarkupSource* ms = (MarkupSource*)source;
  ...
  if (entry != NULL)
    {
      ...
      if (schema_name)
        mateconf_meta_info_set_schema (gcmi, schema_name);
      
      mateconf_meta_info_set_mod_time (gcmi, mtime);
      
      if (mod_user)
        mateconf_meta_info_set_mod_user (gcmi, mod_user);
...
static void          
set_value (MateConfSource      *source,
           const char       *key,
           const MateConfValue *value,
           GError          **err)
{
...
mateconf_entry_from_markup_entry (MarkupEntry *entry,
                               const char **locales)
{
  MateConfValue *value;
  const char *schema_name;
   ...
   */
  mateconf_entry = mateconf_entry_new_nocopy (g_strdup (markup_entry_get_name (entry)),
                                        value);
  mateconf_entry_set_schema_name (mateconf_entry, schema_name);

  return mateconf_entry;
}

static GSList*             
all_entries (MateConfSource *source,
             const char  *key,
             const char **locales,
             GError     **err)
{
  ...
  while (tmp != NULL)
    {
      retval = g_slist_prepend (retval,
                                mateconf_entry_from_markup_entry (tmp->data,
                                                               locales));
...
static GSList*
all_subdirs (MateConfSource *source,
             const char  *key,
             GError     **err)
{
  MarkupSource *ms = (MarkupSource*)source;
...
static void          
unset_value (MateConfSource *source,
             const char  *key,
             const char  *locale,
             GError     **err)
{
...
static gboolean
dir_exists (MateConfSource *source,
            const char  *key,
            GError     **err)
{
  MarkupSource *ms = (MarkupSource*)source;
...
static void          
remove_dir (MateConfSource *source,
            const char  *key,
            GError     **err)
{
...
static void          
set_schema (MateConfSource *source,
            const char  *key,
            const char  *schema_name,
            GError     **err)
{
...
static gboolean      
sync_all (MateConfSource *source,
          GError     **err)
{
  MarkupSource* ms = (MarkupSource*)source;
...
static void          
destroy_source (MateConfSource *source)
{
  ms_destroy ((MarkupSource*)source);
}

static void
clear_cache (MateConfSource *source)
{
  MarkupSource* ms = (MarkupSource*)source;
  ...
  if (!markup_tree_sync (ms->tree, NULL))
    {
      ...
      mateconf_log (GCL_WARNING, "Could not sync data in order to drop cache");
      return;
    }
...
{
  mateconf_log (GCL_DEBUG, _("Initializing Markup backend module"));

  return NULL;
}

G_MODULE_EXPORT MateConfBackendVTable* 
mateconf_backend_get_vtable (void)
{
...
ms_destroy (MarkupSource* ms)
{
   ...
   * situation
   */
  if (ms->lock != NULL && !mateconf_release_lock (ms->lock, &error))
    {
      mateconf_log (GCL_ERR, _("Failed to give up lock on XML directory \"%s\": %s"),
  ...
  if (!g_source_remove (ms->timeout_id))
    {
      ...
      mateconf_log (GCL_ERR, "timeout not found to remove?");
    }
