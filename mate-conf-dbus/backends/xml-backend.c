...
static MateConfSource*  resolve_address (const gchar* address,
                                      GError** err);
...
static MateConfBackendVTable xml_vtable = {
...
{
  mateconf_log(GCL_DEBUG, _("Unloading XML backend module."));
}
...
resolve_address (const gchar* address, GError** err)
{
  ...
  if (!(flags & MATECONF_SOURCE_ALL_READABLE) &&
      !(flags & MATECONF_SOURCE_ALL_WRITEABLE))
    {
      mateconf_set_error(err, MATECONF_ERROR_BAD_ADDRESS, _("Can't read from or write to the XML root directory in the address \"%s\""), address);
      g_free(root_dir);
      return NULL;
    }  
...
query_metainfo  (MateConfSource* source, const gchar* key,
                 GError** err)
{
  XMLSource* xs = (XMLSource*)source;
  gchar* parent;
...
static void          
set_value (MateConfSource* source, const gchar* key, const MateConfValue* value,
           GError** err)
{
  XMLSource* xs = (XMLSource*)source;
  Dir* dir;
                 ...
                 GError** err)
{
  ...
  if (sync_err)
    {
      mateconf_log (GCL_WARNING, _("Error syncing the XML backend directory cache: %s"),
                 sync_err->message);
                 ...
                 GError** err)
{
  ...
  mateconf_log(GCL_DEBUG, "XML backend: unset value `%s'", key);
...
{
  mateconf_log(GCL_DEBUG, _("Initializing XML backend module"));
...
static XMLSource*
xs_new       (const gchar* root_dir, guint dir_mode, guint file_mode, MateConfLock* lock)
{
  XMLSource* xs;
...
xs_destroy   (XMLSource* xs)
{
  ...
  /* do this first in case we're in a "fast cleanup just before exit"
     situation */
  if (xs->lock != NULL && !mateconf_release_lock(xs->lock, &error))
    {
