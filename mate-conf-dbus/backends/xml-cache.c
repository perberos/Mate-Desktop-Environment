  ...
  if (!dir_sync (dir, &deleted, &error))
    {
      ...
      mateconf_log (GCL_ERR, "%s", error->message);
      g_error_free (error);
        ...
        gconstpointer b)
{
  ...
  if (mateconf_key_is_below (key_a, key_b))
    return 1; /* a above b, so b is earlier in the list */
  else if (mateconf_key_is_below (key_b, key_a))
    return -1;
  else
    {
      const char *ap = key_a;
            ...
            GError  **err)
{
  ...
  mateconf_log (GCL_DEBUG, "Syncing the dir cache");
  ...
  if ((cd->now - last_access) >= cd->length)
    {
      if (!dir_sync_pending(dir))
        ...
        {
          mateconf_log(GCL_WARNING, _("Unable to remove directory `%s' from the XML backend cache, because it has not been successfully synced to disk"),
                    dir_get_name(dir));
          return FALSE;
        }
    }
                  ...
                  GError  **err)
{
  ...
  if (dir != NULL)
    {
      mateconf_log(GCL_DEBUG, "Using dir %s from cache", key);
      return dir;
    }
  else
    {
  ...
  if (dir == NULL)
    {
      mateconf_log(GCL_DEBUG, "Creating new dir %s", key);
                         ...
                         const gchar *key)
{
  ...
  parent_key = mateconf_key_directory (key);
              ...
              Dir* d)
{
  ...
  mateconf_log(GCL_DEBUG, "Caching dir %s", dir_get_name(d));
                      ...
                      Dir* dir, gpointer data)
{
  ...
  if (dir_sync_pending (dir))
    mateconf_log(GCL_DEBUG, "Destroying a directory (%s) with sync still pending",
              dir_get_name (dir));
#endif
  dir_destroy (dir);
}
                          ...
                          Dir   *d)
{
  ...
  name = mateconf_key_key (dir_get_name (d));
