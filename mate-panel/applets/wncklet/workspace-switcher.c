...
static void
applet_unrealized (MatePanelApplet *applet,
		   PagerData   *pager)
{
	pager->screen = NULL;
	pager->wm = PAGER_WM_UNKNOWN;
...
static gboolean
applet_scroll (MatePanelApplet    *applet,
...
static void
num_rows_changed (MateConfClient *client,
		  guint        cnxn_id,
		  MateConfEntry  *entry,
		  PagerData   *pager)
{
	...
	if (entry->value != NULL &&
	    entry->value->type == MATECONF_VALUE_INT) {
...
static void
display_workspace_names_changed (MateConfClient *client,
				 guint        cnxn_id,
				 MateConfEntry  *entry,
				 PagerData   *pager)
{
...
static void
all_workspaces_changed (MateConfClient *client,
			guint        cnxn_id,
			MateConfEntry  *entry,
			PagerData   *pager)
{
...
static void
setup_mateconf (PagerData *pager)
{
	...
	key = mate_panel_applet_mateconf_get_full_key (MATE_PANEL_APPLET (pager->applet),
					       "num_rows");
	pager->listeners[0] = mateconf_client_notify_add(client, key,
				(MateConfClientNotifyFunc)num_rows_changed,
				pager,
				NULL, NULL);
	...
	pager->listeners[1] = mateconf_client_notify_add(client, key,
				(MateConfClientNotifyFunc)display_workspace_names_changed,
				pager,
				NULL, NULL);
	...
	pager->listeners[2] = mateconf_client_notify_add(client, key,
				(MateConfClientNotifyFunc)all_workspaces_changed,
				pager,
				NULL, NULL);
...
gboolean
workspace_switcher_applet_fill (MatePanelApplet *applet)
{
	PagerData *pager;
	...
	mate_panel_applet_set_background_widget (MATE_PANEL_APPLET (pager->applet),
	...
	mate_panel_applet_setup_menu_from_file (MATE_PANEL_APPLET (pager->applet),
...
display_workspace_names_toggled (GtkToggleButton *button,
				 PagerData       *pager)
{
	mate_panel_applet_mateconf_set_bool (MATE_PANEL_APPLET (pager->applet),
				     "display_workspace_names",
...
num_rows_value_changed (GtkSpinButton *button,
			PagerData       *pager)
{
	mate_panel_applet_mateconf_set_int (MATE_PANEL_APPLET (pager->applet),
				    "num_rows",
