...
static void
applet_realized (MatePanelApplet  *applet,
		 TasklistData *tasklist)
{
...
static void
applet_change_pixel_size (MatePanelApplet  *applet,
			  gint          size,
			  TasklistData *tasklist)
{
...
static void
display_all_workspaces_changed (MateConfClient  *client,
				guint         cnxn_id,
				MateConfEntry   *entry,
				TasklistData *tasklist)
{
...
get_grouping_type (MateConfValue *value)
{
...
static void
group_windows_changed (MateConfClient  *client,
		       guint         cnxn_id,
		       MateConfEntry   *entry,
		       TasklistData *tasklist)
{
	...
	if (!entry->value ||
	    (entry->value->type != MATECONF_VALUE_BOOL &&
	     entry->value->type != MATECONF_VALUE_STRING))
		return;
...
static void
move_unminimized_windows_changed (MateConfClient  *client,
				  guint         cnxn_id,
				  MateConfEntry   *entry,
				  TasklistData *tasklist)
{
...
static void
setup_mateconf (TasklistData *tasklist)
{
	...
	key = mate_panel_applet_mateconf_get_full_key (MATE_PANEL_APPLET (tasklist->applet),
					       "display_all_workspaces");
	tasklist->listeners[0] = mateconf_client_notify_add(client, key,
				(MateConfClientNotifyFunc)display_all_workspaces_changed,
				tasklist,
				NULL, NULL);
	g_free (key);
	...
	key = mate_panel_applet_mateconf_get_full_key (MATE_PANEL_APPLET (tasklist->applet),
					       "group_windows");
	tasklist->listeners[1] = mateconf_client_notify_add(client, key,
				(MateConfClientNotifyFunc)group_windows_changed,
				tasklist,
				NULL, NULL);
	g_free (key);
	...
	tasklist->listeners[2] = mateconf_client_notify_add(client, key,
				(MateConfClientNotifyFunc)move_unminimized_windows_changed,
				tasklist,
				NULL, NULL);
	g_free (key);
...
gboolean
window_list_applet_fill (MatePanelApplet *applet)
{
	TasklistData *tasklist;
	...
	mate_panel_applet_set_background_widget (MATE_PANEL_APPLET (tasklist->applet),
	...
	mate_panel_applet_setup_menu_from_file (MATE_PANEL_APPLET (tasklist->applet),
		       ...
		       TasklistData    *tasklist)
{
	if (gtk_toggle_button_get_active (button)) {
		...
		mate_panel_applet_mateconf_set_string (MATE_PANEL_APPLET (tasklist->applet),
					       "group_windows", str,
					       NULL);
	}
}
...
move_minimized_toggled (GtkToggleButton *button,
			TasklistData    *tasklist)
{
	mate_panel_applet_mateconf_set_bool (MATE_PANEL_APPLET (tasklist->applet),
				     "move_unminimized_windows",
