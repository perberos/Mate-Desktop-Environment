/* Generic bezier shape item for MateCanvasWidget.  Most code taken
 * from mate-canvas-bpath but made into a shape item.
 *
 * MateCanvas is basically a port of the Tk toolkit's most excellent
...
static void mate_canvas_shape_class_init   (MateCanvasShapeClass *class);
static void mate_canvas_shape_init         (MateCanvasShape      *bpath);
static void mate_canvas_shape_destroy      (GtkObject               *object);
static void mate_canvas_shape_set_property (GObject               *object,
					     guint                  param_id,
					     const GValue          *value,
                                             GParamSpec            *pspec);
static void mate_canvas_shape_get_property (GObject               *object,
					     guint                  param_id,
					     GValue                *value,
                                             GParamSpec            *pspec);

static void   mate_canvas_shape_update      (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags);
static void   mate_canvas_shape_realize     (MateCanvasItem *item);
static void   mate_canvas_shape_unrealize   (MateCanvasItem *item);
static void   mate_canvas_shape_draw        (MateCanvasItem *item, GdkDrawable *drawable,
                                              int x, int y, int width, int height);
static double mate_canvas_shape_point       (MateCanvasItem *item, double x, double y,
                                              int cx, int cy, MateCanvasItem **actual_item);
static void   mate_canvas_shape_render      (MateCanvasItem *item, MateCanvasBuf *buf);
static void   mate_canvas_shape_bounds      (MateCanvasItem *item,
					      double *x1, double *y1, double *x2, double *y2);

static gulong get_pixel_from_rgba (MateCanvasItem *item, guint32 rgba_color);
static guint32 get_rgba_from_color (GdkColor * color);
static void set_gc_foreground (GdkGC *gc, gulong pixel);
static void gcbp_ensure_gdk (MateCanvasShape * bpath);
static void gcbp_destroy_gdk (MateCanvasShape * bpath);
static void set_stipple (GdkGC *gc, GdkBitmap **internal_stipple, GdkBitmap *stipple, int reconfigure);
static void gcbp_ensure_mask (MateCanvasShape * bpath, gint width, gint height);
static void gcbp_draw_ctx_unref (GCBPDrawCtx * ctx);

static MateCanvasItemClass *parent_class;

GType
mate_canvas_shape_get_type (void)
{
	static GType shape_type;

	if (!shape_type) {
		const GTypeInfo object_info = {
			sizeof (MateCanvasShapeClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) mate_canvas_shape_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (MateCanvasShape),
			0,			/* n_preallocs */
			(GInstanceInitFunc) mate_canvas_shape_init,
			NULL			/* value_table */
		};

		shape_type = g_type_register_static (MATE_TYPE_CANVAS_ITEM, "MateCanvasShape",
						     &object_info, 0);
	}
...
static void
mate_canvas_shape_class_init (MateCanvasShapeClass *class)
{
	...
	/* when this gets checked into libmateui, change the
           GTK_TYPE_POINTER to GTK_TYPE_MATE_CANVAS_SHAPE, and add an
           entry to mate-boxed.defs */

	gobject_class->set_property = mate_canvas_shape_set_property;
	gobject_class->get_property = mate_canvas_shape_get_property;
	...
	object_class->destroy = mate_canvas_shape_destroy;

	item_class->update = mate_canvas_shape_update;
	item_class->realize = mate_canvas_shape_realize;
	item_class->unrealize = mate_canvas_shape_unrealize;
	item_class->draw = mate_canvas_shape_draw;
	item_class->point = mate_canvas_shape_point;
	item_class->render = mate_canvas_shape_render;
	item_class->bounds = mate_canvas_shape_bounds;
}

static void
mate_canvas_shape_init (MateCanvasShape *shape)
{
	shape->priv = g_new (MateCanvasShapePriv, 1);
...
static void
mate_canvas_shape_destroy (GtkObject *object)
{
	MateCanvasShape *shape;
	MateCanvasShapePriv *priv;

	g_return_if_fail (object != NULL);
	g_return_if_fail (MATE_IS_CANVAS_SHAPE (object));
	...
	if (shape->priv) {
		...
		if (priv->path) mate_canvas_path_def_unref (priv->path);
...
/**
 * mate_canvas_shape_set_path_def:
 * @shape: a MateCanvasShape
 * @def: a MateCanvasPathDef 
 *
 * This function sets the the MateCanvasPathDef used by the
 * MateCanvasShape. Notice, that it does not request updates, as
 * it is meant to be used from item implementations, from inside
 * update queue.
 */
 
void
mate_canvas_shape_set_path_def (MateCanvasShape *shape, MateCanvasPathDef *def) 
{
	MateCanvasShapePriv *priv;

	g_return_if_fail (shape != NULL);
	g_return_if_fail (MATE_IS_CANVAS_SHAPE (shape));

	priv = shape->priv;

	if (priv->path) {
		mate_canvas_path_def_unref (priv->path);
		priv->path = NULL;
	}

	if (def) {
		priv->path = mate_canvas_path_def_duplicate (def);
	}
}

static void
mate_canvas_shape_set_property (GObject      *object,
                                 guint         param_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
	...
	case PROP_FILL_COLOR:
		if (mate_canvas_get_color (item->canvas, g_value_get_string (value), &color)) {
			priv->fill_set = TRUE;
...
static void
get_color_value (MateCanvasShape *shape, gulong pixel, GValue *value)
{
	MateCanvas *canvas = MATE_CANVAS_ITEM (shape)->canvas;
...
/**
 * mate_canvas_shape_get_path_def:
 * @shape: a MateCanvasShape
 *
 * This function returns the #MateCanvasPathDef that the shape
 * currently uses.  It adds a reference to the #MateCanvasPathDef and
 * returns it, if there is not a #MateCanvasPathDef set for the shape
 * it returns NULL.
 *
 * Returns: a #MateCanvasPathDef or NULL if none is set for the shape.
 */
 
MateCanvasPathDef *
mate_canvas_shape_get_path_def (MateCanvasShape *shape)
{
	MateCanvasShapePriv *priv;
	
	g_return_val_if_fail (shape != NULL, NULL);
	g_return_val_if_fail (MATE_IS_CANVAS_SHAPE (shape), NULL);

	priv = shape->priv;

	if (priv->path) {
		mate_canvas_path_def_ref (priv->path);
		return priv->path;
	}
...
static void
mate_canvas_shape_get_property (GObject     *object,
                                 guint        param_id,
                                 GValue      *value,
                                 GParamSpec  *pspec)
{
	MateCanvasItem         *item = MATE_CANVAS_ITEM (object);
	MateCanvasShape        *shape = MATE_CANVAS_SHAPE (object);
	MateCanvasShapePriv    *priv = shape->priv;
	MateCanvasShapePrivGdk *gdk;
...
static void
mate_canvas_shape_realize (MateCanvasItem *item)
{
	MateCanvasShape *shape;

	shape = MATE_CANVAS_SHAPE (item);
...
static void
mate_canvas_shape_unrealize (MateCanvasItem *item)
{
	MateCanvasShape *shape;
...
static void
mate_canvas_shape_render (MateCanvasItem *item,
			     MateCanvasBuf *buf)
{
	MateCanvasShape *shape;
...
static void
mate_canvas_shape_draw (MateCanvasItem *item,
	...
	int height)
{
	...
	if (priv->fill_set) {
		...
		if (gdk->fill_stipple) mate_canvas_set_stipple_origin (item->canvas, gdk->fill_gc);
	...
	if (priv->outline_set) {
		...
		if (gdk->outline_stipple) mate_canvas_set_stipple_origin (item->canvas, gdk->outline_gc);
		/* Draw subpaths */
...
static void
mate_canvas_shape_ensure_gdk_points (MateCanvasShapePrivGdk *gdk, gint num)
{
...
static void
mate_canvas_shape_update_gdk (MateCanvasShape * shape, double * affine, ArtSVP * clip, int flags)
{
	MateCanvasShapePriv * priv;
	MateCanvasShapePrivGdk * gdk;
	...
	if (priv->path) {
		MateCanvasPathDef * apath, * cpath, * opath;
		ArtBpath * abpath;
		GSList * clist, * olist;
		gint pos;
...
#if 0
		/* Allocate array */
		gdk->num_points = mate_canvas_path_def_length (priv->path) * 1000 - 1;
		gdk->points = g_new (GdkPoint, gdk->num_points);
		g_print ("Points %d\n", gdk->num_points);
		/* Transform path */
#endif

		abpath = art_bpath_affine_transform (mate_canvas_path_def_bpath (priv->path), affine);
		apath = mate_canvas_path_def_new_from_bpath (abpath);

		/* Split path into open and closed parts */

		cpath = mate_canvas_path_def_closed_parts (apath);
		opath = mate_canvas_path_def_open_parts (apath);
		mate_canvas_path_def_unref (apath);

		/* Split partial paths into subpaths */

		clist = mate_canvas_path_def_split (cpath);
		mate_canvas_path_def_unref (cpath);
		olist = mate_canvas_path_def_split (opath);
		mate_canvas_path_def_unref (opath);
		...
		while (clist) {
			...
			path = (MateCanvasPathDef *) clist->data;
			bpath = mate_canvas_path_def_bpath (path);
			vpath = art_bez_path_to_vec (bpath, 0.1);
			for (len = 0; vpath[len].code != ART_END; len++) ;

			mate_canvas_shape_ensure_gdk_points (gdk, len);
			for (i = 0; i < len; i++) {
		...
		while (olist) {
			...
			path = (MateCanvasPathDef *) olist->data;
			bpath = mate_canvas_path_def_bpath (path);
			vpath = art_bez_path_to_vec (bpath, 0.1);
			for (len = 0; vpath[len].code != ART_END; len++) ;
	...
	if (bbox_set) {
		...
		mate_canvas_update_bbox (MATE_CANVAS_ITEM (shape),
					  x1, y1,
					  x2 + 1, y2 + 1);
	}
...
static void
mate_canvas_shape_update (MateCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	MateCanvasShape * shape;
	MateCanvasShapePriv * priv;
	ArtSVP * svp;
	...
	if ((priv->fill_set) && (priv->path) && (mate_canvas_path_def_any_closed (priv->path))) {
		MateCanvasPathDef * cpath;
		ArtSvpWriter *swr;
		ArtVpath *vpath;
		ArtBpath *abp;
		ArtSVP *svp2;
		...
		cpath = mate_canvas_path_def_closed_parts (shape->priv->path);
		abp = art_bpath_affine_transform (mate_canvas_path_def_bpath (cpath), affine);
		mate_canvas_path_def_unref (cpath);
	...
	if (priv->outline_set && priv->path && !mate_canvas_path_def_is_empty (priv->path)) {
		gdouble width;
		ArtBpath * abp;
		ArtVpath * vpath;
		...
		abp = art_bpath_affine_transform (mate_canvas_path_def_bpath (priv->path), affine);
		...
		svp = art_svp_vpath_stroke (vpath,
					    mate_canvas_join_gdk_to_art (priv->join),
					    mate_canvas_cap_gdk_to_art (priv->cap),
					    width,
					    priv->miterlimit,
					    0.25);
		art_free (vpath);
		...
		if (item->canvas->aa) {
			/* Update clipped */
			mate_canvas_item_update_svp_clip (item, &priv->outline_svp, svp, clip_path);
		} else {
			if (priv->outline_svp) {
	...
	if (!item->canvas->aa) {
		mate_canvas_shape_update_gdk (shape, affine, clip_path, flags);
	}
}

static double
mate_canvas_shape_point (MateCanvasItem *item, double x, double y,
			    int cx, int cy, MateCanvasItem **actual_item)
{
	MateCanvasShape *shape;
	double dist;
...
static gulong
get_pixel_from_rgba (MateCanvasItem *item, guint32 rgba_color)
{
	return mate_canvas_get_color_pixel (item->canvas, rgba_color);
}
...
static void
gcbp_ensure_gdk (MateCanvasShape * shape)
{
	g_assert (!((MateCanvasItem *) shape)->canvas->aa);
	...
	if (!shape->priv->gdk) {
		...
		gdk = g_new (MateCanvasShapePrivGdk, 1);

		gdk->fill_pixel = get_pixel_from_rgba ((MateCanvasItem *) shape, shape->priv->fill_rgba);
		gdk->outline_pixel = get_pixel_from_rgba ((MateCanvasItem *) shape, shape->priv->outline_rgba);
...
static void
gcbp_destroy_gdk (MateCanvasShape * shape)
{
	MateCanvasShapePrivGdk * gdk;
...
static void
gcbp_ensure_mask (MateCanvasShape * shape, gint width, gint height)
{
	MateCanvasShapePrivGdk * gdk;
	GCBPDrawCtx * ctx;
	...
	if (!ctx) {
		...
		canvas = MATE_CANVAS_ITEM (shape)->canvas;
	...
	if ((width > ctx->width) || (height > ctx->height)) {
		...
		window = ((GtkWidget *) (((MateCanvasItem *) shape)->canvas))->window;
...
static void
mate_canvas_shape_bounds (MateCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	MateCanvasShape * shape;
	MateCanvasShapePriv * priv;
	ArtDRect bbox;
	...
	if (priv->outline_set && priv->path && !mate_canvas_path_def_is_empty (priv->path)) {
		...
		vpath = art_bez_path_to_vec (mate_canvas_path_def_bpath (priv->path), 0.1);
		...
		art_svp_free (svp);
	} else if ((priv->fill_set) && (priv->path) && (mate_canvas_path_def_any_closed (priv->path))) {
		MateCanvasPathDef *cpath;
		ArtSvpWriter *swr;
		ArtVpath *vpath;
		ArtSVP *svp2;
		...
		cpath = mate_canvas_path_def_closed_parts (shape->priv->path);
		/* Render, until SVP */
		vpath = art_bez_path_to_vec (mate_canvas_path_def_bpath (cpath), 0.1);
		mate_canvas_path_def_unref (cpath);
