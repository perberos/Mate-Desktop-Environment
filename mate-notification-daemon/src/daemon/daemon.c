...
static void
on_popup_location_changed (MateConfClient  *client,
                           guint         cnxn_id,
                           MateConfEntry   *entry,
                           NotifyDaemon *daemon)
{
...
notify_daemon_init (NotifyDaemon *daemon)
{
        ...
        location = mateconf_client_get_string (client,
                                            MATECONF_KEY_POPUP_LOCATION,
                                            NULL);
        daemon->priv->stack_location = get_stack_location_from_string (location);
        g_free (location);

        daemon->priv->mateconf_cnxn = mateconf_client_notify_add (client,
                                                            MATECONF_KEY_POPUP_LOCATION,
                                                            (MateConfClientNotifyFunc) on_popup_location_changed,
                                                            daemon,
                                                            NULL,
                                                            NULL);
...
screensaver_active (GtkWidget *nw)
{
        ...
        if (!dbus_g_proxy_call (gs_proxy,
                                ...
                                &active,
                                G_TYPE_INVALID)) {
                g_warning ("Failed to call mate-screensaver: %s",
                           error->message);
                g_error_free (error);
        }
                              ...
                              DBusGMethodInvocation *context)
{
        ...
        sound_enabled = mateconf_client_get_bool (mateconf_client,
                                               MATECONF_KEY_SOUND_ENABLED,
                                               NULL);
