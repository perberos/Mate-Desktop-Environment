...
static int shutdown_mateconfd = FALSE;
static int ping_mateconfd = FALSE;
static int spawn_mateconfd = FALSE;
...
static const GOptionEntry server_entries[] = {
  ...
  { 
    ...
    G_OPTION_ARG_NONE,
    &shutdown_mateconfd,
    N_("Shut down mateconfd. DON'T USE THIS OPTION WITHOUT GOOD REASON."),
    NULL
  },
  { 
    "ping",
    ...
    G_OPTION_ARG_NONE,
    &ping_mateconfd,
    N_("Return 0 if mateconfd is running, 2 if not."),
    NULL
  },
  { 
    "spawn",
    ...
    G_OPTION_ARG_NONE,
    &spawn_mateconfd,
    N_("Launch the configuration server (mateconfd). (Normally happens automatically when needed.)"),
    NULL
  },
  {
    NULL
...
static const GOptionEntry install_entries[] = {
  ...
  },
  {
    ...
    G_OPTION_ARG_NONE,
    &use_local_source,
    N_("Bypass server, and access the configuration database directly. Requires that mateconfd is not running."),
    NULL
  },
  {
    ...
    N_("Properly installs schema files on the command line into the database. Specify a custom configuration source in the MATECONF_CONFIG_SOURCE environment variable, or set set the variable to an empty string to use the default configuration source."),
    NULL
  },
  {
    ...
    N_("Properly uninstalls schema files on the command line from the database. MATECONF_CONFIG_SOURCE environment variable should be set to a non-default configuration source or set to the empty string to use the default."),
    NULL
  },
  {
    NULL
...
static int do_break_key(MateConfEngine* conf, const gchar** args);
static int do_break_directory(MateConfEngine* conf, const gchar** args);
static int do_makefile_install(MateConfEngine* conf, const gchar** args, gboolean unload);
static int do_recursive_list(MateConfEngine* conf, const gchar** args);
static int do_search_key(MateConfEngine* conf, const gchar** args);
static int do_search_key_regex(MateConfEngine* conf, const gchar** args);
static int do_dump_values(MateConfEngine* conf, const gchar** args);
static int do_all_pairs(MateConfEngine* conf, const gchar** args);
static void list_pairs_in_dir(MateConfEngine* conf, const gchar* dir, guint depth);
static int get_schema_from_xml(xmlNodePtr node, gchar **schema_key, GHashTable** schemas_hash, GSList **applyto_list);
static int get_first_value_from_xml(xmlNodePtr node, MateConfValue** ret_value);
static void print_value_in_xml(MateConfValue* value, int indent);
static void dump_entries_in_dir(MateConfEngine* conf, const gchar* dir, const gchar *base_dir);
static gboolean do_dir_exists(MateConfEngine* conf, const gchar* dir);
static void do_spawn_daemon(MateConfEngine* conf);
static int do_get(MateConfEngine* conf, const gchar** args);
static int do_set(MateConfEngine* conf, const gchar** args);
static int do_toggle(MateConfEngine* conf, const gchar** args);
static int do_get_type(MateConfEngine* conf, const gchar** args);
static int do_get_list_size(MateConfEngine* conf, const gchar** args);
static int do_get_list_element(MateConfEngine* conf, const gchar** args);
static int do_set_schema(MateConfEngine* conf, const gchar** args);
static int do_all_entries(MateConfEngine* conf, const gchar** args);
static int do_unset(MateConfEngine* conf, const gchar** args);
static int do_recursive_unset (MateConfEngine* conf, const gchar** args);
static int do_all_subdirs(MateConfEngine* conf, const gchar** args);
static int do_load_file(MateConfEngine* conf, LoadType load_type, gboolean unload, const gchar* file, const gchar** base_dirs);
static int do_sync(MateConfEngine* conf);
static int do_short_docs (MateConfEngine *conf, const gchar **args);
static int do_long_docs (MateConfEngine *conf, const gchar **args);
static int do_get_schema_name (MateConfEngine *conf, const gchar **args);
static int do_associate_schema (MateConfEngine *conf, const gchar **args);
static int do_dissociate_schema (MateConfEngine *conf, const gchar **args);
...
main (int argc, char** argv)
{
  ...
  context = g_option_context_new (N_("- Tool to manipulate a MateConf configuration"));
  ...
  if (ping_mateconfd && (shutdown_mateconfd || set_mode || get_mode || unset_mode ||
                      all_subdirs_mode || all_entries_mode || recursive_list || 
                      get_type_mode || get_list_size_mode || get_list_element_mode ||
                      spawn_mateconfd || dir_exists || schema_file ||
  ...
  if (dir_exists && (shutdown_mateconfd || set_mode || get_mode || unset_mode ||
                     all_subdirs_mode || all_entries_mode || recursive_list || search_key || search_key_regex ||
                     get_type_mode || get_list_size_mode || get_list_element_mode ||
                     spawn_mateconfd || schema_file ||
                     makefile_install_mode || makefile_uninstall_mode ||
                     break_key_mode || break_dir_mode || short_docs_mode ||
                         long_docs_mode || schema_name_mode))
    {
  ...
  if (schema_file && (shutdown_mateconfd || set_mode || get_mode || unset_mode ||
                      all_subdirs_mode || all_entries_mode || recursive_list || search_key || search_key_regex ||
                      get_type_mode || get_list_size_mode || get_list_element_mode ||
                      spawn_mateconfd || dir_exists ||
                      makefile_install_mode || makefile_uninstall_mode ||
                      break_key_mode || break_dir_mode || short_docs_mode ||
                         long_docs_mode || schema_name_mode))
    {
  ...
  if (makefile_install_mode && (shutdown_mateconfd || set_mode || get_mode || unset_mode ||
                                all_subdirs_mode || all_entries_mode || recursive_list || search_key || search_key_regex ||
                                get_type_mode || get_list_size_mode || get_list_element_mode ||
                                makefile_uninstall_mode ||
                                spawn_mateconfd || dir_exists || schema_file ||
  ...
  if (makefile_uninstall_mode && (shutdown_mateconfd || set_mode || get_mode ||
  ...
  if (break_key_mode && (shutdown_mateconfd || set_mode || get_mode || unset_mode ||
                                all_subdirs_mode || all_entries_mode || recursive_list || search_key || search_key_regex ||
                                get_type_mode || get_list_size_mode || get_list_element_mode ||
                                spawn_mateconfd || dir_exists || schema_file ||
                                makefile_install_mode || makefile_uninstall_mode ||
  ...
  if (break_dir_mode && (shutdown_mateconfd || set_mode || get_mode || unset_mode ||
                                all_subdirs_mode || all_entries_mode || recursive_list || search_key || search_key_regex ||
                                get_type_mode || get_list_size_mode || get_list_element_mode ||
                                spawn_mateconfd || dir_exists || schema_file ||
  ...
  if (!mateconf_init(argc, argv, &err))
    {
      g_printerr (_("Failed to init MateConf: %s\n"), err->message);
      g_error_free(err);
      err = NULL;
      return 1;
    }
  ...
  if (makefile_install_mode &&
      g_getenv ("MATECONF_DISABLE_MAKEFILE_SCHEMA_INSTALL"))
    {
      g_print (_("MATECONF_DISABLE_MAKEFILE_SCHEMA_INSTALL is set, not installing schemas\n"));
      makefile_install_mode = FALSE;
    }

  if (makefile_uninstall_mode &&
      g_getenv ("MATECONF_DISABLE_MAKEFILE_SCHEMA_UNINSTALL"))
    {
      g_print (_("MATECONF_DISABLE_MAKEFILE_SCHEMA_UNINSTALL is set, not uninstalling schemas\n"));
  ...
  if (makefile_install_mode || makefile_uninstall_mode)
    {
      ...
      if (config_source == NULL)
        {
          g_printerr (_("Must set the MATECONF_CONFIG_SOURCE environment variable\n"));
          return 1;
        }
  ...
  if (config_source == NULL)
      ...
      if (g_getenv ("DBUS_SESSION_BUS_ADDRESS") == NULL)
        {
          ...
          conf = mateconf_engine_get_local_for_addresses (addresses, &err);
  ...
  else
    {
      ...
      addresses = mateconf_persistent_name_get_address_list (config_source);

      if (use_local_source)
        conf = mateconf_engine_get_local_for_addresses (addresses, &err);
      else
        conf = mateconf_engine_get_for_addresses (addresses, &err);
  ...
  if (shutdown_mateconfd)
    {
      err = NULL;
      mateconf_shutdown_daemon(&err);
    }
...
static void 
recurse_subdir_list(MateConfEngine* conf, GSList* subdirs, const gchar* parent, guint depth)
{
  GSList* tmp;
  gchar* whitespace;
  ...
  while (tmp != NULL)
    {
      ...
      recurse_subdir_list(conf, mateconf_engine_all_dirs(conf, s, NULL), s, depth+1);
...
static int
do_recursive_list(MateConfEngine* conf, const gchar** args)
{
  if (args == NULL)
    {
  ...
  while (*args)
    {
      ...
      subdirs = mateconf_engine_all_dirs(conf, *args, NULL);
...
static void 
search_key_in_dir(MateConfEngine* conf, const gchar* dir, MatchFunc match_func, gpointer match_data)
{
  GSList* pairs;
  GSList* tmp;
  GError* err = NULL;
  
  pairs = mateconf_engine_all_entries(conf, dir, &err);
  ...
  if (pairs != NULL)
    {
      ...
      while (tmp != NULL)
        {
          MateConfEntry* pair = tmp->data;
          gchar* s;
	  const gchar *k;

          if (mateconf_entry_get_value (pair) && 
	      (!ignore_schema_defaults || !mateconf_entry_get_is_default (pair)))
            s = mateconf_value_to_string (mateconf_entry_get_value (pair));
          else
            s = g_strdup(_("(no value set)"));

	  k = mateconf_key_key (mateconf_entry_get_key (pair));
...
static void 
recurse_subdir_search(MateConfEngine* conf, GSList* subdirs, const gchar* parent,
  ...
  while (tmp != NULL)
    {
      ...
      recurse_subdir_search(conf, mateconf_engine_all_dirs(conf, s, NULL), s, match_func, match_data);
...
static int
do_search(MateConfEngine* conf, MatchFunc match_func, gpointer match_data)
{
  GSList* subdirs;

  subdirs = mateconf_engine_all_dirs(conf, "/", NULL);
...
static int
do_search_key(MateConfEngine* conf, const gchar** args)
{
...
static int
do_search_key_regex(MateConfEngine* conf, const gchar** args)
{
  GRegex* regex;
  GError *error = NULL;
...
static void 
recurse_subdir_dump(MateConfEngine* conf, GSList* dirs, const gchar* base_dir)
{
  GSList* tmp;
  ...
  while (tmp != NULL)
    {
      ...
      subdirs = g_slist_sort(mateconf_engine_all_dirs(conf, s, NULL),
			     (GCompareFunc)strcmp);
...
static int
do_dump_values(MateConfEngine* conf, const gchar** args)
{
  if (args == NULL)
    {
  ...
  while (*args)
    {
      ...
      subdirs = g_slist_sort(mateconf_engine_all_dirs(conf, *args, NULL),
			     (GCompareFunc)strcmp);
...
static void 
list_pairs_in_dir(MateConfEngine* conf, const gchar* dir, guint depth)
{
  GSList* pairs;
  GSList* tmp;
  gchar* whitespace;
      ...
      while (tmp != NULL)
        {
          ...
          g_print (" %s%s = %s\n", whitespace,
		   mateconf_key_key (mateconf_entry_get_key (pair)),
		   s);
...
static int
do_all_pairs(MateConfEngine* conf, const gchar** args)
{      
  while (*args)
    {
      list_pairs_in_dir(conf, *args, 0);
 ...
 */
static void
print_schema_in_xml(MateConfValue* value, int indent)
{
  ...
  list_type = mateconf_schema_get_list_type(schema);
  car_type = mateconf_schema_get_car_type(schema);
  cdr_type = mateconf_schema_get_cdr_type(schema);

  owner = mateconf_schema_get_owner(schema);
  default_value = mateconf_schema_get_default_value(schema);
  short_desc = mateconf_schema_get_short_desc(schema);
  long_desc = mateconf_schema_get_long_desc(schema);
  ...
  g_print ("%s  <type>%s</type>\n", whitespace, mateconf_value_type_to_string(type));

  if (type == MATECONF_VALUE_LIST)
    g_print ("%s  <list_type>%s</list_type>\n", whitespace, mateconf_value_type_to_string(list_type));
  else if (type == MATECONF_VALUE_PAIR)
    {
      g_print ("%s  <car_type>%s</car_type>\n", whitespace, mateconf_value_type_to_string(car_type));
      g_print ("%s  <cdr_type>%s</cdr_type>\n", whitespace, mateconf_value_type_to_string(cdr_type));
    }

  g_print ("%s  <locale name=\"%s\">\n", whitespace, mateconf_schema_get_locale (schema));
...
static void
print_pair_in_xml(MateConfValue* value, int indent)
{
  gchar* whitespace;
  ...
  g_print ("%s  <car>\n", whitespace);
  print_value_in_xml(mateconf_value_get_car(value), indent + 4);
  g_print ("%s  </car>\n", whitespace);

  g_print ("%s  <cdr>\n", whitespace);
  print_value_in_xml(mateconf_value_get_cdr(value), indent + 4);
  g_print ("%s  </cdr>\n", whitespace);
...
static void
print_list_in_xml(MateConfValue* value, int indent)
{
  MateConfValueType list_type;
  GSList* tmp;
  gchar* whitespace;
  ...
  tmp = mateconf_value_get_list(value);
  while (tmp)
    {
...
static void
print_value_in_xml(MateConfValue* value, int indent)
{
  gchar* whitespace;
  gchar* tmp;
  ...
  switch (value->type)
    ...
    case MATECONF_VALUE_INT:
      tmp = mateconf_value_to_string(value);
      g_print ("%s  <int>%s</int>\n", whitespace, tmp);
      g_free(tmp);
      break;
    ...
    case MATECONF_VALUE_STRING:
      tmp = g_markup_escape_text(mateconf_value_get_string(value), -1);
...
static int
compare_entries (MateConfEntry* a, MateConfEntry *b)
{
  return strcmp(mateconf_entry_get_key(a), mateconf_entry_get_key(b));
}

static void 
dump_entries_in_dir(MateConfEngine* conf, const gchar* dir, const gchar* base_dir)
{
  GSList* entries;
  GSList* tmp;
  GError* err = NULL;
  
  entries = g_slist_sort(mateconf_engine_all_entries(conf, dir, &err),
  ...
  while (tmp != NULL)
    {
      ...
      g_print ("      <key>%s</key>\n",
	       get_key_relative(mateconf_entry_get_key(entry), base_dir));
      ...
      if (mateconf_entry_get_schema_name(entry))
        g_print ("      <schema_key>%s</schema_key>\n",
		 get_key_relative(mateconf_entry_get_schema_name(entry), base_dir));

      if (entry->value && 
	  (!ignore_schema_defaults || !mateconf_entry_get_is_default(entry)))
...
static gboolean
do_dir_exists(MateConfEngine* conf, const gchar* dir)
{
  GError* err = NULL;
  gboolean exists = FALSE;
  
  exists = mateconf_engine_dir_exists(conf, dir_exists, &err);
...
static void
do_spawn_daemon(MateConfEngine* conf)
{
  GError* err = NULL;

  if (!mateconf_spawn_daemon(&err))
    {
      g_printerr (_("Failed to spawn the configuration server (mateconfd): %s\n"), 
		  err->message);
      g_error_free(err);
      err = NULL;
    }
...
static inline MateConfValue *
get_maybe_without_default (MateConfEngine  *conf,
			   const gchar  *key,
			   GError      **err)
{
  if (!ignore_schema_defaults)
    {
      return mateconf_engine_get (conf, key, err);
    }
  else
    {
      return mateconf_engine_get_without_default (conf, key, err);
    }
}

static int
do_get(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
      ...
      if (value != NULL)
        {
          if (value->type != MATECONF_VALUE_SCHEMA)
            {
              s = mateconf_value_to_string(value);
            ...
            {
              MateConfSchema* sc = mateconf_value_get_schema(value);
              MateConfValueType stype = mateconf_schema_get_type(sc);
              MateConfValueType slist_type = mateconf_schema_get_list_type(sc);
              MateConfValueType scar_type = mateconf_schema_get_car_type(sc);
              MateConfValueType scdr_type = mateconf_schema_get_cdr_type(sc);
              const gchar* long_desc = mateconf_schema_get_long_desc(sc);
              const gchar* short_desc = mateconf_schema_get_short_desc(sc);
              const gchar* owner = mateconf_schema_get_owner(sc);
              MateConfValue* def_value = mateconf_schema_get_default_value(sc);
              
              g_print (_("Type: %s\n"), mateconf_value_type_to_string(stype));
              g_print (_("List Type: %s\n"), mateconf_value_type_to_string(slist_type));
              g_print (_("Car Type: %s\n"), mateconf_value_type_to_string(scar_type));
              g_print (_("Cdr Type: %s\n"), mateconf_value_type_to_string(scdr_type));
              if (def_value)
                s = mateconf_value_to_string (def_value);
              else
                s = NULL;
...
static int
do_set(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
  ...
  while (*args)
    {
      ...
      if (value_list_type != NULL)
        {
          ...
          if (list_type == MATECONF_VALUE_INVALID)
            return 1;
	}
      if (value_car_type != NULL)
        {
          car_type = read_value_type(value_car_type);
          if (car_type == MATECONF_VALUE_INVALID)
            return 1;
	}
      if (value_cdr_type != NULL)
        {
          cdr_type = read_value_type(value_cdr_type);
          if (cdr_type == MATECONF_VALUE_INVALID)
            return 1;
	}

      if (type == MATECONF_VALUE_SCHEMA)
        {
      ...
      if (type == MATECONF_VALUE_LIST &&
          list_type != MATECONF_VALUE_STRING &&
          list_type != MATECONF_VALUE_INT &&
          list_type != MATECONF_VALUE_FLOAT &&
          list_type != MATECONF_VALUE_BOOL)
        {
          g_printerr (_("When setting a list you must specify a primitive list-type\n"));
          return 1;
	}

      if (type == MATECONF_VALUE_PAIR &&
          ((car_type != MATECONF_VALUE_STRING &&
            car_type != MATECONF_VALUE_INT &&
            car_type != MATECONF_VALUE_FLOAT &&
            car_type != MATECONF_VALUE_BOOL) ||
           (cdr_type != MATECONF_VALUE_STRING &&
            cdr_type != MATECONF_VALUE_INT &&
            cdr_type != MATECONF_VALUE_FLOAT &&
            cdr_type != MATECONF_VALUE_BOOL)))
        {
      ...
      if (type == MATECONF_VALUE_LIST)
	gval = mateconf_value_new_list_from_string(list_type, value, &err);
      else if (type == MATECONF_VALUE_PAIR)
	gval = mateconf_value_new_pair_from_string(car_type, cdr_type, value, &err);
      else
        gval = mateconf_value_new_from_string(type, value, &err);
      ...
      mateconf_engine_set (conf, key, gval, &err);
  ...
  mateconf_engine_suggest_sync(conf, &err);
...
static int
do_toggle(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
  ...
  while (*args)
    {
      ...
      mateconf_value_set_bool (value, !mateconf_value_get_bool (value));

      mateconf_engine_set (conf, key, value, &err);
...
static int
do_get_type(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
...
static int
do_get_list_size(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
  MateConfValue *list = NULL;
  ...
  if (list->type != MATECONF_VALUE_LIST)
    {
      g_printerr (_("Key %s is not a list.\n"), *args);
      return 1;
    }

  g_print ("%u\n", g_slist_length (mateconf_value_get_list (list)));

  return 0;
}

static int
do_get_list_element(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
  MateConfValue *list = NULL, *element = NULL;
  GSList *iter = NULL;
  gchar* s = NULL;
  int idx = 0;
  ...
  iter = mateconf_value_get_list (list);
  ...
  s = mateconf_value_to_string (element);
  g_print ("%s\n", s);
  g_free (s);
...
static int
do_schema_info (MateConfEngine *conf, const gchar **args,
                int info)
{
  GError* err = NULL;
  ...
  while (*args)
    {
      ...
      entry = mateconf_engine_get_entry (conf, *args, NULL, TRUE, &err);
                  ...
                  if (schema)
                    {
                      if (info == SCHEMA_INFO_SHORT_DOCS)
                        docs = mateconf_schema_get_short_desc (schema);
                      else if (info == SCHEMA_INFO_LONG_DOCS)
                        docs = mateconf_schema_get_long_desc (schema);
                    }
...
static int
do_short_docs (MateConfEngine *conf, const gchar **args)
{
  return do_schema_info (conf, args, SCHEMA_INFO_SHORT_DOCS);
}

static int
do_long_docs (MateConfEngine *conf, const gchar **args)
{
  return do_schema_info (conf, args, SCHEMA_INFO_LONG_DOCS);
}

static int
do_get_schema_name (MateConfEngine *conf, const gchar **args)
{
  return do_schema_info (conf, args, SCHEMA_INFO_SCHEMA_NAME);
}

static int
do_associate_schema (MateConfEngine *conf, const gchar **args)
{
  GError *err;
  ...
  err = NULL;
  if (!mateconf_engine_associate_schema (conf, args[1], args[0], &err))
    {
...
static int
do_dissociate_schema (MateConfEngine *conf, const gchar **args)
{
  GError *err;
  gboolean failed;
  ...
  while (*args)
    {
      err = NULL;
      if (!mateconf_engine_associate_schema (conf, *args, NULL, &err))
        {
...
static int
do_set_schema(MateConfEngine* conf, const gchar** args)
{
  MateConfSchema* sc;
  MateConfValue* val;
  const gchar* key;
  ...
  val = mateconf_value_new(MATECONF_VALUE_SCHEMA);
  ...
  if (short_desc)
    mateconf_schema_set_short_desc(sc, short_desc);

  if (long_desc)
    mateconf_schema_set_long_desc(sc, long_desc);

  if (owner)
    mateconf_schema_set_owner(sc, owner);
  ...
  mateconf_engine_set (conf, key, val, &err);
...
static int
do_all_entries(MateConfEngine* conf, const gchar** args)
{
  if (args == NULL)
    {
...
static int
do_unset(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
  ...
  while (*args)
    {
      err = NULL;
      mateconf_engine_unset(conf, *args, &err);
  ...
  err = NULL;
  mateconf_engine_suggest_sync(conf, NULL); /* ignore errors */

  return 0;
}

static int
do_recursive_unset (MateConfEngine* conf, const gchar** args)
{
  if (args == NULL)
    {
  ...
  while (*args)
    {
      ...
      err = NULL;
      mateconf_engine_recursive_unset (conf, *args,
...
static int
do_all_subdirs(MateConfEngine* conf, const gchar** args)
{
  GError* err = NULL;
  ...
  while (*args)
    {
      ...
      subdirs = mateconf_engine_all_dirs(conf, *args, &err);
...
static int
get_list_value_from_xml(xmlNodePtr node, MateConfValue** ret_value)
{
  xmlNodePtr iter;
  ...
  list_type = mateconf_value_type_from_string(list_type_str);
          ...
          if (get_first_value_from_xml(iter, &element) == 0)
            {
              if (element->type == list_type)
			    ...
			    mateconf_value_type_to_string (element->type));
            }
        }
...
static inline void
get_car_cdr_value(xmlNodePtr node, MateConfValue** ret_value)
{
  xmlNodePtr iter;
...
static int
get_pair_value_from_xml(xmlNodePtr node, MateConfValue** ret_value)
{
  xmlNodePtr iter;
  MateConfValue* value = NULL;
  ...
  if (car && cdr)
    {
      ...
      mateconf_value_set_car(value, car);
      mateconf_value_set_cdr(value, cdr);
    }
  else
    {
      ...
      if (strcmp((char *)iter->name, "int") == 0)
        {
          if ((content = (char *)xmlNodeGetContent(iter)) != NULL)
            {
              value = mateconf_value_new_from_string(MATECONF_VALUE_INT, content, &error);
              if (value == NULL)
                {
                  g_assert(error != NULL);
      ...
      else if (strcmp((char *)iter->name, "float") == 0)
        {
          if ((content = (char *)xmlNodeGetContent(iter)) != NULL)
            {
              value = mateconf_value_new_from_string(MATECONF_VALUE_FLOAT, content, &error);
              if (value == NULL)
                {
                  g_assert(error != NULL);
      ...
      else if (strcmp((char *)iter->name, "string") == 0)
        {
          if ((content = (char *)xmlNodeGetContent(iter)) != NULL)
            {
              value = mateconf_value_new_from_string(MATECONF_VALUE_STRING, content, &error);
              if (value == NULL)
                {
                  g_assert(error != NULL);
      ...
      else if (strcmp((char *)iter->name, "bool") == 0)
        {
          if ((content = (char *)xmlNodeGetContent(iter)) != NULL)
            {
              value = mateconf_value_new_from_string(MATECONF_VALUE_BOOL, content, &error);
              if (value == NULL)
                {
                  g_assert(error != NULL);
 ...
 */
static int
get_first_value_from_xml(xmlNodePtr node, MateConfValue** ret_value)
{
  MateConfValue* value = NULL;
  GSList* values;
  GSList* tmp;
...
static void
set_values(MateConfEngine* conf, gboolean unload, const gchar* base_dir, const gchar* key, const char* schema_key, GSList* values)
{
  GSList* tmp;
  gchar* full_key;
 
  if (base_dir)
    full_key = mateconf_concat_dir_and_key(base_dir, key);
  else
      ...
      if (!unload)
        {
          if (base_dir && *schema_key != '/')
            full_schema_key = mateconf_concat_dir_and_key(base_dir, schema_key);
          else
            full_schema_key = g_strdup(schema_key);
        }

      if (!mateconf_engine_associate_schema(conf, full_key, full_schema_key,  &error))
        {
          g_assert(error != NULL);
  ...
  while (tmp)
    {
      MateConfValue* value = tmp->data;
      GError* error;

      error = NULL;
      if (!unload)
        mateconf_engine_set(conf, full_key, value, &error);
      else
...
static int
process_entry(MateConfEngine* conf, gboolean unload, xmlNodePtr node, const gchar** base_dirs, const char* orig_base)
{
  xmlNodePtr iter;
  GSList* values = NULL;
  GSList* tmp;
                         ...
                         MateConfValue** retloc)
{
  ...
  switch (info->type)
    ...
    case MATECONF_VALUE_LIST:
      {
        GError* error = NULL;
        if (info->list_type == MATECONF_VALUE_INVALID)
	  {
            g_printerr (_("WARNING: invalid or missing list_type for schema (%s)\n"),
			info->key);
            break;
	  }
        *retloc = mateconf_value_new_list_from_string(info->list_type,
						   default_value,
						   &error);
        if (*retloc == NULL)
          {
    ...
    case MATECONF_VALUE_PAIR:
      {
        GError* error = NULL;
        if (info->car_type == MATECONF_VALUE_INVALID ||
	    info->cdr_type == MATECONF_VALUE_INVALID)
	  {
            g_printerr (_("WARNING: invalid or missing car_type or cdr_type for schema (%s)\n"),
                    info->key);
            break;
	  }
        *retloc = mateconf_value_new_pair_from_string(info->car_type,
						   info->cdr_type,
						   default_value,
						   &error);
        if (*retloc == NULL)
    ...
    case MATECONF_VALUE_FLOAT:
      {
        GError* error = NULL;
        *retloc = mateconf_value_new_from_string(info->type,
                                              default_value,
                                              &error);
        if (*retloc == NULL)
          {
    ...
    default:
      g_printerr (_("WARNING: mateconftool internal error, unknown MateConfValueType\n"));
      break;
    }
      ...
      if (iter->type == XML_ELEMENT_NODE)
        {
          ...
          if (strcmp((char *)iter->name, "key") == 0)
          ...
          else if (strcmp((char *)iter->name, "type") == 0)
            {
              ...
              if (tmp)
                {
                  info->type = mateconf_value_type_from_string(tmp);
                  if (info->type == MATECONF_VALUE_INVALID)
          ...
          else if (strcmp((char *)iter->name, "list_type") == 0)
            {
              ...
              if (tmp)
                {
                  info->list_type = mateconf_value_type_from_string(tmp);
                  if (info->list_type != MATECONF_VALUE_INT &&
		      info->list_type != MATECONF_VALUE_FLOAT &&
		      info->list_type != MATECONF_VALUE_STRING &&
		      info->list_type != MATECONF_VALUE_BOOL)
		    {
		      info->list_type = MATECONF_VALUE_INVALID;
                      g_printerr (_("WARNING: list_type can only be int, float, string or bool and not `%s'\n"),
				  tmp);
		    }
                  else if (info->list_type == MATECONF_VALUE_INVALID)
                    g_printerr (_("WARNING: failed to parse type name `%s'\n"),
				tmp);
    		  xmlFree(tmp);
                }
          ...
          else if (strcmp((char *)iter->name, "car_type") == 0)
            {
              ...
              if (tmp)
                {
                  info->car_type = mateconf_value_type_from_string(tmp);
                  if (info->car_type != MATECONF_VALUE_INT &&
		      info->car_type != MATECONF_VALUE_FLOAT &&
		      info->car_type != MATECONF_VALUE_STRING &&
		      info->car_type != MATECONF_VALUE_BOOL)
		    {
		      info->car_type = MATECONF_VALUE_INVALID;
                      g_printerr (_("WARNING: car_type can only be int, float, string or bool and not `%s'\n"),
				  tmp);
		    }
                  else if (info->car_type == MATECONF_VALUE_INVALID)
                    g_printerr (_("WARNING: failed to parse type name `%s'\n"),
				tmp);
                  xmlFree(tmp);
                }
          ...
          else if (strcmp((char *)iter->name, "cdr_type") == 0)
            {
              ...
              if (tmp)
                {
                  info->cdr_type = mateconf_value_type_from_string(tmp);
                  if (info->cdr_type != MATECONF_VALUE_INT &&
		      info->cdr_type != MATECONF_VALUE_FLOAT &&
		      info->cdr_type != MATECONF_VALUE_STRING &&
		      info->cdr_type != MATECONF_VALUE_BOOL)
		    {
		      info->cdr_type = MATECONF_VALUE_INVALID;
                      g_printerr (_("WARNING: cdr_type can only be int, float, string or bool and not `%s'\n"),
				  tmp);
		    }
                  else if (info->cdr_type == MATECONF_VALUE_INVALID)
                    g_printerr (_("WARNING: failed to parse type name `%s'\n"),
				tmp);
                  xmlFree(tmp);
                }
...
process_locale_info(xmlNodePtr node, SchemaInfo* info)
{
  ...
  mateconf_schema_set_locale(schema, name);

  /* Only set the global default on the C locale */
  if (strcmp(name, "C") == 0 && info->global_default != NULL)
    mateconf_schema_set_default_value(schema, info->global_default);

  /* Fill in the global info */
  if (info->type != MATECONF_VALUE_INVALID)
    mateconf_schema_set_type(schema, info->type);

  if (info->list_type != MATECONF_VALUE_INVALID)
    mateconf_schema_set_list_type(schema, info->list_type);

  if (info->car_type != MATECONF_VALUE_INVALID)
    mateconf_schema_set_car_type(schema, info->car_type);

  if (info->cdr_type != MATECONF_VALUE_INVALID)
    mateconf_schema_set_cdr_type(schema, info->cdr_type);

  if (info->owner != NULL)
    mateconf_schema_set_owner(schema, info->owner);

  if (info->gettext_domain != NULL)
    mateconf_schema_set_gettext_domain(schema, info->gettext_domain);
      ...
      if (iter->type == XML_ELEMENT_NODE)
        {
          if (strcmp((char *)iter->name, "default_value") == 0)
                  ...
                  if (strcmp((char *)tmp_node->name, "value") == 0)
                    {
                      ...
                      if (val)
                        mateconf_schema_set_default_value_nocopy(schema, val);
                      break;
                    }
                  tmp_node = tmp_node->next;
                }
          ...
          else if (strcmp((char *)iter->name, "short") == 0)
            {
              ...
              if (tmp != NULL)
                {
                  mateconf_schema_set_short_desc(schema, tmp);
                  xmlFree(tmp);
                }
            }
          else if (strcmp((char *)iter->name, "long") == 0)
            {
              ...
              if (tmp != NULL)
                {
                  mateconf_schema_set_long_desc(schema, tmp);
                  xmlFree(tmp);
                }
            }
          else
  ...
  g_hash_table_insert(info->hash,
                      (gchar*)mateconf_schema_get_locale(schema), /* cheat to save copying this string */
                      schema);
...
static int
process_key_list(MateConfEngine* conf, gboolean unload, const gchar* schema_name, GSList* keylist)
{
  GSList* tmp;
  ...
  while (tmp != NULL)
    {
      ...
      if (!mateconf_engine_associate_schema(conf, tmp->data, !unload ? schema_name : NULL, &error))
        {
          g_assert(error != NULL);
...
get_schema_from_xml(xmlNodePtr node, gchar **schema_key, GHashTable** schemas_hash, GSList **applyto_list)
{
  ...
  info.key = NULL;
  info.type = MATECONF_VALUE_INVALID;
  info.list_type = MATECONF_VALUE_INVALID;
  info.car_type = MATECONF_VALUE_INVALID;
  info.cdr_type = MATECONF_VALUE_INVALID;
  info.apply_to = NULL;
  ...
  if (info.type == MATECONF_VALUE_INVALID)
    {
      ...
      if (info.global_default)
        mateconf_value_free(info.global_default);
...
hash_install_foreach(gpointer key, gpointer value, gpointer user_data)
{
  ...
  if (!info->unload)
    {
      if (!mateconf_engine_set_schema(info->conf, info->key, schema, &error))
	{
	  g_assert(error != NULL);

	  g_printerr (_("WARNING: failed to install schema `%s', locale `%s': %s\n"),
		      info->key, mateconf_schema_get_locale(schema), error->message);
	  g_error_free(error);
	  error = NULL;
	}
      else
	{
	  ...
	  g_print (_("Installed schema `%s' for locale `%s'\n"),
		   info->key, mateconf_schema_get_locale(schema));
	}
    }
  else
    {
      if (!mateconf_engine_unset (info->conf, info->key, &error))
	{
	  g_assert(error != NULL);
...
static int
process_schema(MateConfEngine* conf, gboolean unload, xmlNodePtr node)
{
  struct {
    MateConfEngine* conf;
    gboolean unload;
...
static int
process_list(MateConfEngine* conf, LoadType load_type, gboolean unload, xmlNodePtr node, const gchar** base_dirs)
{
...
static int
do_load_file(MateConfEngine* conf, LoadType load_type, gboolean unload, const gchar* file, const gchar** base_dirs)
{
#define LOAD_TYPE_TO_ROOT(t) ((t == LOAD_SCHEMA_FILE) ? "mateconfschemafile" : "mateconfentryfile")
#define LOAD_TYPE_TO_LIST(t) ((t == LOAD_SCHEMA_FILE) ? "schemalist" : "entrylist")
...
static int
do_makefile_install(MateConfEngine* conf, const gchar** args, gboolean unload)
{
  int retval = 0;
...
static gboolean
key_breakage(MateConfEngine* conf, const gchar* key, BreakagePhase phase)
{
  GError* error = NULL;
  
  if (phase == BreakageCleanup)
    {
      mateconf_engine_unset(conf, key, &error);
      if (error != NULL)
        {
  ...
  else if (phase == BreakageSetBadValues)
    {
      ...
      mateconf_engine_set_string(conf, key, "", &error);
      if (check_err(&error))
        return FALSE;
      
      mateconf_engine_set_string(conf, key, "blah blah blah 93475028934670 @%^%$&%$&^%", &error);
      if (check_err(&error))
        return FALSE;
      
      mateconf_engine_set_bool(conf, key, TRUE, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_bool(conf, key, FALSE, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_float(conf, key, 100.0, &error);
      if (check_err(&error))
        return FALSE;
      ...
      mateconf_engine_set_float(conf, key, 0.0, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_int(conf, key, 0, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_int(conf, key, 5384750, &error);
      if (check_err(&error))
        return FALSE;
      
      mateconf_engine_set_int(conf, key, -11, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_list(conf, key, MATECONF_VALUE_BOOL, list, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_pair(conf, key, MATECONF_VALUE_INT, MATECONF_VALUE_BOOL,
                            &an_int, &a_bool, &error);
      if (check_err(&error))
        return FALSE;

      mateconf_engine_set_pair(conf, key, MATECONF_VALUE_FLOAT, MATECONF_VALUE_STRING,
                            &a_float, &a_string, &error);
      if (check_err(&error))
        return FALSE;

      /* empty pair */
      val = mateconf_value_new(MATECONF_VALUE_PAIR);
      mateconf_engine_set (conf, key, val, &error);
      mateconf_value_free(val);
      if (check_err(&error))
        return FALSE;

      list = NULL;
      mateconf_engine_set_list(conf, key, MATECONF_VALUE_STRING, list, &error);
      if (check_err(&error))
        return FALSE;
      mateconf_engine_set_list(conf, key, MATECONF_VALUE_INT, list, &error);
      if (check_err(&error))
        return FALSE;
      mateconf_engine_set_list(conf, key, MATECONF_VALUE_BOOL, list, &error);
      if (check_err(&error))
...
static int
do_break_key(MateConfEngine* conf, const gchar** args)
{
  if (args == NULL)
    {
...
static int
do_break_directory(MateConfEngine* conf, const gchar** args)
{
  if (args == NULL)
    {
  ...
  while (*args)
    {
      ...
      while (i < 10)
        {
          keys[i] = mateconf_unique_key();
          full_keys[i] = mateconf_concat_dir_and_key(*args, keys[i]);
...
do_get_default_source (const gchar** args)
{
  ...
  /* Try with $sysmateconfdir/schema-install-source */
  filename = g_build_filename (MATECONF_ETCDIR, "schema-install-source", NULL);
  ...
  source = g_strconcat("xml:merged:", MATECONF_ETCDIR, "/mateconf.xml.defaults", NULL);
  g_print ("%s\n", source);
  g_free(source);
